...
class Evaluation(models.Model):
    ...
    number_of_projects = models.IntegerField(
        ...
        help_text="Minimum number of projects required is 2",
    )
    ...
    def __str__(self):
        """
        Return a string representation of the Evaluation instance.
        """
        return self.name
...
class Project(models.Model):
    ...
    def calculate_npv(self):
        ...
        cash_flows = [-self.initial_investment] + list(
            cash_flows_input
        )
        # calucalte the NPV for all cashflow starting including the initial investment
        npv_value = npf.npv(
            self.evaluation.discount_rate / 100, cash_flows
        )
...
def calculate_annualized_npv(self):
    """
    Calculate the Annualized Net Present Value (NPV) for the project
    NPV is positive.
    """
    if self.npv >= 0:
        try:
...
class Annual_net_cashflow(models.Model):
    """
    Model representing annual net cashflows for a project.
    """
    ...
    project = models.ForeignKey(
        ...
        related_name="annual_net_cashflows",
    )

    def __str__(self):
        '''
        Return a string representation of the Annual_net_cashflow instance.
        '''
        return f"Cashflow for {self.project.name}"